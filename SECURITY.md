...
- **Role allow-list** — `VALID_ROLES` (required env var) defines accepted Tier-1 IdP actor classes (`operator`, `clinician`, `patient`). Unknown roles are rejected fail-closed ([CWE-863](https://cwe.mitre.org/data/definitions/863.html)). The service refuses to start if `VALID_ROLES` is unset or empty. Platform service-management APIs (`/api/services/*`) require JWT role `operator` (see [UPGRADE_NOTES.md](UPGRADE_NOTES.md)).
