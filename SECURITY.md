...
- **Actor allow-list** — `VALID_ACTORS` (required env var) defines accepted Tier-1 IdP actor classes (`operator`, `study`, `clinician`, `patient`). Unknown actors are rejected fail-closed ([CWE-863](https://cwe.mitre.org/data/definitions/863.html)). The service refuses to start if `VALID_ACTORS` is unset or empty. Platform service-management APIs (`/api/services/*`) require JWT role `operator` (see [INSTALLATION_PLAN.md](INSTALLATION_PLAN.md)).
- **Tenant-type vocabulary** — org kinds accepted in `neosofia:tenant_type` come from required env var **`VALID_TENANT_TYPES`**. Unknown types are omitted from the JWT at mint.
