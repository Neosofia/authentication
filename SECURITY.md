...
- **Role allow-list** — the `VALID_ROLES` env var (required; comma-separated) defines the accepted WorkOS org membership roles. Unknown roles are rejected fail-closed, preventing privilege escalation via provider-side role injection ([CWE-863](https://cwe.mitre.org/data/definitions/863.html)). Users with no org membership are also rejected outright. The service refuses to start if `VALID_ROLES` is unset or empty.
