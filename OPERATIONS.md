...
3. Under **Authorization** (environment/instance roles), create Tier-1 actor classes: `operator`, `clinician`, and `patient`. Assign `operator` to registry admins in each test organization (see [INSTALLATION_PLAN.md](INSTALLATION_PLAN.md)).
...
| `VALID_ACTORS` | Comma-separated Tier-1 IdP actor classes at token issuance (required): `operator,study,clinician,patient`. See [INSTALLATION_PLAN.md](INSTALLATION_PLAN.md). | — |
