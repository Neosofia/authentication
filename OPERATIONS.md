...
4. Select `PDC Test Clinic`, assign role **clinician**.

> **Note**: Clinicians require an explicit org role assignment. Patients authenticate without
> org membership and will receive `pdc:user_type=patient` with no `pdc:tenant_id` claim.
...
| `JWT_ISSUER` | Set to your issuer URL | `https://auth.pdc.local` |
| `ACCESS_TOKEN_TTL_SECS` | Optional, default `900` | — |
| `DATABASE_URL` | Local Postgres | `postgresql+asyncpg://pdc:dev_only@localhost:5014/pdc_auth` |
