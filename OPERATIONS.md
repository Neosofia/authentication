...
3. Under **Authorization** (environment/instance roles), create Tier-1 actor classes: `operator`, `clinician`, and `patient`. Assign `operator` to registry admins in each test organization (see [UPGRADE_NOTES.md](UPGRADE_NOTES.md)).
...
| `VALID_ACTORS` | Comma-separated Tier-1 IdP actor classes at token issuance (required): `operator,study,clinician,patient`. See [UPGRADE_NOTES.md](UPGRADE_NOTES.md). | — |
