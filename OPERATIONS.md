...
3. Under **Authorization** (environment/instance roles), create Tier-1 actor classes: `operator`, `clinician`, and `patient`. Assign `operator` to registry admins in each test organization (see [RELEASE_NOTES.md](RELEASE_NOTES.md)).
...
| `VALID_ROLES` | Comma-separated Tier-1 IdP actor classes at token issuance (required): `operator,clinician,patient`. See [RELEASE_NOTES.md](RELEASE_NOTES.md). | — |
