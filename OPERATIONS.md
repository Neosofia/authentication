...
Set `tenant_type` to the org kind for that organization. Valid kinds are configured via **`VALID_TENANT_TYPES`** (comma-separated, required). For a single local test org, `platform` is typical. If omitted from the template, the service infers `tenants.type` from the first provisioned org-role slug (e.g. `platform.admin` → `platform`). `patient.self` does not infer a tenant type; enrolled patients belong to a **site** tenant.
...
All users must have organization membership and a Tier-1 actor in `VALID_ACTORS`.

### Greenfield Step 0 (tier-2 registry roles)

Login provision creates the user registry row with **`roles: []`**. Before platform admin UI or operator registry APIs work, ops must assign tier-2 slugs (e.g. `platform.admin`) on the user service database, then the admin **logs out and back in** so this service refreshes its JWT roles mirror. Tier-1 **`operator`** in WorkOS does not imply **`platform.admin`**. See [INSTALLATION_PLAN — Step 0](INSTALLATION_PLAN.md#greenfield-step-0--assign-platform-registry-roles).
...
3. Under **Authorization** (environment/instance roles), create Tier-1 actor classes: `operator`, `study`, `clinician`, `patient`, and **`demo`**. Assign `operator` to registry admins in each test organization (see [INSTALLATION_PLAN.md](INSTALLATION_PLAN.md)).

> [!IMPORTANT]
> **WorkOS environment vs platform `VALID_ACTORS`.** WorkOS **Staging** and **Production** are separate environments — each has its own Authorization roles *and* its own API keys. Creating a `demo` role in Staging does nothing for local AuthKit unless you also create it in the WorkOS environment that matches your `WORKOS_API_KEY`. Likewise, Authentication must list `demo` in **`VALID_ACTORS`** for that deploy (local `.authentication.env`, compose override, or cloud secret). If WorkOS emits `demo` but `VALID_ACTORS` omits it, token issuance strips the actor and the CDP demo bootstrap never runs.

#### Demo workspace humans

Demo humans need **three tier-1 WorkOS roles**, not just `demo`:

| WorkOS role | Purpose |
|-------------|---------|
| **`demo`** | Bootstrap only. CDP UI detects `demo` in `neosofia:actors`, assigns tier-2 `patient.self` + `site.clinical`, and copies care-episode data from template **DEMO-123**. Not shown in the session role menu. |
| **`patient`** | Required after bootstrap to open **Patient** views (`patient.self` → patient actor). |
| **`clinician`** | Required after bootstrap to open **Clinician** views (`site.clinical` → clinician actor). |

`demo` does **not** replace `patient` or `clinician`. Bootstrap writes the tier-2 slugs, but the session menu only offers Patient / Site Clinical when the JWT includes the matching tier-1 actors (see CDP `buildSessionRoleChoices`). Add `operator` or `study` only if that human should use those menus too.

After first bootstrap, the user should **sign out and back in** so tier-2 roles refresh in the platform JWT.
...
| `AUTHENTICATION_CLIENT_SECRET` | Plaintext client secret generated during env setup (or provisioned manually by the operator); migration `002` hashes this exact value into `service_credentials` and fails if it is blank | `scripts/setup-env.py` / secret store |
