...
def test_identity_payload_requires_tenant_uuid():
    with pytest.raises(ValueError, match="tenant_uuid"):
        user_provisioning._identity_payload(_identity(tenant_uuid=None))
...
def test_provision_user_registry_sync_caches_roles_on_success(monkeypatch):
    mock_db = MagicMock()
    mock_session = MagicMock()
    mock_session.return_value.__enter__.return_value = mock_db
    mock_response = MagicMock(status_code=200)
    mock_response.json.return_value = {"roles": ["platform.admin"]}
    ...
    monkeypatch.setattr(user_provisioning, "issue_service_token", MagicMock(return_value="jwt"))
    monkeypatch.setattr(user_provisioning.httpx, "put", MagicMock(return_value=mock_response))
    cache_mock = MagicMock()
    monkeypatch.setattr(user_provisioning, "cache_roles_mirror", cache_mock)

    assert user_provisioning._provision_user_registry_sync(_identity()) is True
    cache_mock.assert_called_once()


def test_provision_user_registry_sync_continues_when_roles_cache_fails(monkeypatch):
    mock_session = MagicMock()
    mock_session.return_value.__enter__.return_value = MagicMock()
...
def test_provision_user_registry_sync_returns_false_on_exception(monkeypatch):
    monkeypatch.setattr(user_provisioning.settings, "authentication_client_secret", "secret")
    monkeypatch.setattr(
        user_provisioning,
        "get_service",
...
def test_provision_user_registry_starts_background_thread(monkeypatch):
    monkeypatch.setattr(user_provisioning.settings, "user_provisioning_enabled", True)
    thread_mock = MagicMock()
    monkeypatch.setattr(user_provisioning.threading, "Thread", thread_mock)

    user_provisioning.provision_user_registry(_identity())

    thread_mock.assert_called_once()
    assert thread_mock.return_value.start.called