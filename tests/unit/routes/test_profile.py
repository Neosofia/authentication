...
_PROFILE_ID = "019e02b4-47e1-778a-9331-476e9f927bd9"
_PROFILE_URL = f"/api/v1/profiles/{_PROFILE_ID}"
...
def test_profile_expired_token(mock_decode, client):
    ...
    response = client.get(_PROFILE_URL, headers={"Authorization": "Bearer 123"})
    assert response.status_code == 401
    assert response.json["error"] == "unauthenticated"
...
def test_profile_missing_sub_claim(mock_decode, client):
    ...
    assert response.json == {"error": "authorization_unavailable"}
...
def test_profile_db_fetch_user_failed(mock_decode, mock_db_session, mock_log, client):
    mock_decode.return_value = {"sub": _PROFILE_ID}
    mock_db = MagicMock()
...
def test_profile_db_fetch_org_missing(mock_decode, mock_db_session, client):
    mock_decode.return_value = {
        "sub": _PROFILE_ID,
        "neosofia:tenant_uuid": "019e02e1-94e1-722b-bd61-f7f95fb1604c",
    }
